...
export default function Header() {
  ...
  return (
    <>
      {/* Navigation Desktop */}
      <header className="hidden sm:flex justify-between items-center p-3 px-8 border-b border-gray-300">
        <div className="text-2xl font-bold hover:cursor-pointer">Logo</div>

        <nav className="flex gap-10">
          <Link to="/" className="text-black no-underline hover:text-blue-500 transition-all duration-300">Home</Link>
          <Link to="/products" className="text-black no-underline hover:text-blue-500 transition-all duration-300">Products</Link>
          <Link to="/cart" className="text-black no-underline hover:text-blue-500 transition-all duration-300">Cart</Link>
          {userInfo && userInfo.isAdmin && (
            <Link to="/dashboard" className="text-black no-underline hover:text-blue-500 transition-all duration-300">Dashboard</Link>
          )}
        </nav>
        ...
        {menuOpen && (
          <div className="flex flex-col bg-gray-300 gap-4 p-4">
            <nav className="flex flex-col gap-2">
              ...
              {userInfo && userInfo.isAdmin && (
                <Link to="/dashboard" className="text-black no-underline hover:font-bold hover:relative hover:left-1 transition-all duration-300">Dashboard</Link>
              )}
            </nav>
            <div className="mt-4">
              {userInfo ? (
                <>
                  <Link to="/profile" className="flex items-center gap-2 text-black hover:font-bold">
                    <FaRegUser className="h-5" />
                    <b>{userInfo.username}</b>
                  </Link>
                  <button onClick={logoutHandler} 
                  className=" flex  text-red-500 hover:text-red-400 mt-2">
                  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6">
                  <path d="M12 20C7.58172 20 4 16.4183 4 12C4 7.58172 7.58172 4 12 4" stroke="#ff0000" strokeWidth="1.5" strokeLinecap="round"></path>
                  <path d="M10 12H20M20 12L17 9M20 12L17 15" stroke="#ff0000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path>
                </svg>
                ...
                <>
                  <Link to="/login" className="flex text-indigo-600 hover:text-indigo-400">Login</Link>
                  <Link to="/register" className="text-indigo-600 hover:text-indigo-400 mt-2">Register</Link>
                </>
              )}
            </div>
          </div>
